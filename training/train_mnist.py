...
from dit.dit import DiT_tiny, DiT_small
import os
...
class TrainingConfig:
    ...
    num_workers: int = 4
    output_dir: str = "./outputs/mnist_dit_checkpoints_flow"
    save_model_epochs: int = 2
    objective: str = "flow_matching" # 'ddpm' or 'flow_matching'
    hf_repo_id: str = "NLTuan/mnist_dit_flow" # e.g., 'username/my-mnist-dit'
...
def evaluate_and_save_video(model, config, epoch, device):
    ...
    if config.objective == "ddpm":
        ...
        for t in scheduler.timesteps:
            ...
            # If grayscale, PIL needs it to be 2D or we convert to 'L' if it's 1 channel.
            # make_grid actually outputs 3 channels even for grayscale. So it's fine to keep it as RGB.
